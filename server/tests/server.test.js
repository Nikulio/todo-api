...
describe("DELETE /todos/:id", () => {
  ...
  it("should not remove a todo if not auth", done => {
...
describe("PATCH /todos/:id", () => {
  ...
  it("should not update the todo if not auth", done => {
    var hexId = todos[0]._id.toHexString();
    var text = "This should be the new text";
    request(app)
      .patch(`/todos/${hexId}`)
...
describe("GET /users/me", () => {
  it("should return users if auth", function(done) {
    request(app)
      .get("/users/me")
      .set("x-auth", users[0].tokens[0].token)
      .expect(200)
  ...
  it("should return 401 if not auth", function(done) {
    request(app)
      .get("/users/me")
      .expect(401)
      .expect(res => {
    ...
    request(app)
      ...
      .end(err => {
        ...
        User.findOne({ email })
          .then(user => {
            expect(user).toExist();
            expect(user.password).toNotBe(pass);
            done();
          })
          .catch(e => done(e));
      });
  });
  it("should return valid errors if req invalid", function(done) {
    request(app)
      .post("/users")
      .send()
      .expect(400)
